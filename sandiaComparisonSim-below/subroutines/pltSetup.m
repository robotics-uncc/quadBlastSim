...
for i = 1:tmpN
    rmat = [1 sin(phi(i))*tan(theta(i)) cos(phi(i))*tan(theta(i)); 
         0 cos(phi(i)) -sin(phi(i));
         0 sin(phi(i))/cos(theta(i)) cos(phi(i))/cos(theta(i))];
    tmpRates = rmat*[p(i); q(i); r(i)];
...
% Calculate the impulse over time
% impulseBody   = cumtrapz(t, bodyBlastForces, 1);
% impulseMotor1 = cumtrapz(t, motor1BlastForces, 1);
% impulseMotor2 = cumtrapz(t, motor2BlastForces, 1);
% impulseMotor3 = cumtrapz(t, motor3BlastForces, 1);
% impulseMotor4 = cumtrapz(t, motor4BlastForces, 1);
impulseBody   = cumtrapz(t, bodyPressures, 2);
impulseMotor1 = cumtrapz(t, motor1Pressures, 2);
impulseMotor2 = cumtrapz(t, motor2Pressures, 2);
impulseMotor3 = cumtrapz(t, motor3Pressures, 2);
impulseMotor4 = cumtrapz(t, motor4Pressures, 2);
...
save(append(figDir,'Data.mat'),'t', 'X', 'Y', 'Z', 'XDot', 'YDot', 'ZDot', 'phiB', 'thetaB', 'psi', 'theta', 'phi', 'psiDot', 'thetaDot', 'phiDot', 'p', 'q', 'r', ...
    ...
    'impulseBody', 'impulseMotor1', 'impulseMotor2', 'impulseMotor3', 'impulseMotor4')
