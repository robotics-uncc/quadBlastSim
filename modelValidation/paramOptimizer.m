...
for i = 1:dN
    ...
    t0_i(i) = blastData.times(maxPsIdx);
    ...
    [Ps_i(i), tau_i(i)] = sadovskiy(W,scaledDist(i));

    % Pack the parameters into an IC input for the optimization
    modelPParams = [Ps_i(i), t0_i(i), tau_i(i)];
    PparamIC = [PsStd,t0_i(i),1/200];
    ...
    [Vs_i(i), alpha_i(i), beta_i(i), a_i(i), ~] = deweyParams(scaledDist(i));
    DT = t-t0;
    ...
    % Pack the parameters into an IC input for the optimization
    VparamIC = [Vs_i(i), alpha_i(i), beta_i(i), a_i(i)];

    % Calculate pressure
    modelP(i,:) = Pmodel(modelPParams,t);

    % Calculate velocity
    modelV(i,:) = Vmodel(VparamIC,scaledTime,hval);
    ...
    Ps_opt(i) = optParamP(1);
    t0_opt(i) = optParamP(2);
    tau_opt(i) = optParamP(3);

    %% Calculate the pressure model based on the optimized parameters
    optP(i,:) = Pmodel(optParamP,blastData.times);
    ...
    subplot(1,3,subplotN(i))
    fntSize = 16;
    ax.FontSize = fntSize;
    plot(t*1000,0.001*blastData.overpressure(blastData.nearIdx,:),'DisplayName','CFD','LineWidth',lw,'LineStyle','-.','Color','k')
    hold on
    plot(t*1000,0.001*modelP(i,:), 'DisplayName','Model','LineWidth',lw,'LineStyle',':', 'Color','r')
    plot(t*1000,0.001*optP(i,:),'DisplayName','Fit','LineWidth',lw,'LineStyle','--', 'Color','b')
    grid on
    xlabel('$t$ (ms)')
    if i == 1
        ylabel('$P_{0}$ (kPa)')
    ...
    % Unpack to store the params
    Vs_opt(i) = optParamV(1);
    alpha_opt(i) = optParamV(2);
    beta_opt(i) = optParamV(3);
    a_opt(i) = optParamV(4);

    %% Calculate the velocity model based on the optimized parameters
    optV(i,:) = Vmodel(optParamV,scaledTime,hval);
    ...
    hold on
    plot(t*1000,modelV(i,:)*mach2ms, 'DisplayName','Model','LineWidth',lw,'LineStyle',':', 'Color','r')
    plot(t*1000,optV(i,:)*mach2ms,'DisplayName','Fit','LineWidth',lw,'LineStyle','--', 'Color','b')
    grid on
    xlabel('$t$ (ms)')
    if i == 1
        ylabel('$V$ (m/s)')
    ...
    % end
    yticks(linspace(0,round(max(modelV(i,:)*mach2ms),-2),5))
    xlim([0, tf*1000])
...
for i = 1:dN
    ...
    fntSize = 16;
    ax.FontSize = fntSize;
    plot(t*1000,0.001*modelP(i,:), 'DisplayName','Model Input','LineWidth',lw,'LineStyle',':', 'Color',colors(i))
    hold on
    grid on
    xlabel('$t$ (ms)')
    ...
    subplot(1,2,2)
    ax.FontSize = fntSize;
    plot(t*1000,modelV(i,:)*mach2ms, 'DisplayName','Model Input','LineWidth',lw,'LineStyle',':', 'Color',colors(i))
    hold on
    grid on
    xlabel('$t$ (ms)')
    title(sprintf('$c$ = %2.2f m/s',c),'Interpreter','latex')
    if i == 1
        ylabel('$V$ (m/s)')
        yticks(linspace(0,max(modelV(i,:)*mach2ms),5))
        xticks(linspace(0, tf*1000,5))
        xlim([0, tf*1000])
    end
    % if bbb == 1
...
for i = 1:dN
    ...
    %     fntSize = 16;
    %     ax.FontSize = fntSize;
    %     plot(t*1000,0.001*blastData.overpressure(blastData.nearIdx,:),'LineWidth',lw,'LineStyle',':', 'Color',colors(i))
    %     hold on
    %     grid on
    %     xlabel('$t$ (ms)')
    %     % title(sprintf('$c$ = %2.2f m/s',c),'Interpreter','latex')
    ...
    %     subplot(1,3,2)
    %     ax.FontSize = fntSize;
    %     plot(t*1000,blastData.Umag(blastData.nearIdx,:),'LineWidth',lw,'LineStyle',':', 'Color',colors(i),'DisplayName',append('$r =$ ',string(dVals(i)), ' m'))
    %     hold on
    %     grid on
    %     xlabel('$t$ (ms)')
    %     % title(sprintf('$c$ = %2.2f m/s',c),'Interpreter','latex')
...
fntSize = 16;
ax.FontSize = fntSize;
plot(t*1000,0.001*blastData.overpressure(blastData.nearIdx,:),'LineWidth',lw,'LineStyle',':', 'Color',colors(i),'DisplayName',append('$r =$ ',string(dVals(i)), ' m'))
hold on
grid on
xlabel('$t$ (ms)')
% title(sprintf('$c$ = %2.2f m/s',c),'Interpreter','latex')
