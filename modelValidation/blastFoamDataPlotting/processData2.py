...
for i in range(N):
    ...
    # test = data[desData]
    overpressure[:, i], u0[:, i], u1[:, i], u2[:, i], x[:, i], y[:, i] = data[desData].values.T
    print(str(i) + ' worked')
...
op.to_csv(processedDFN + "overpressure.csv", header=False, index=False)

# umag
mag = pd.DataFrame(np.hypot(u0,u1,u2))
mag.to_csv(processedDFN + "Umag.csv", header=False, index=False)

# time
time = pd.DataFrame(times)
time.to_csv(processedDFN + "times.csv", header=False, index=False)

# radii
radii = pd.DataFrame(np.hypot(x,y))
radii.to_csv(processedDFN + "radii.csv", header=False, index=False)
