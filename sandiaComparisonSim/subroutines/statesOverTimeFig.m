...
pressureFig = figure(7);
pressureFig.Visible = 'off';
set(pressureFig,'Color','w','Units','inches','Position',[0 0 16 6])
set(groot, 'defaultAxesTickLabelInterpreter','latex'); set(groot, 'defaultLegendInterpreter','latex'); set(groot, 'defaultTextInterpreter','latex');

% Make a matrix of all plotted variables`
plotVars1 = [bodyPressures(1,:); motor1Pressures(1,:); motor2Pressures(1,:); motor3Pressures(1,:); motor4Pressures(1,:)];
plotVars2 = [bodyPressures(end,:); motor1Pressures(end,:); motor2Pressures(end,:); motor3Pressures(end,:); motor4Pressures(end,:)];
titleLabs = ["Body Pressure", "Motor 1 Pressure", "Motor 2 Pressure", "Motor 3 Pressure", "Motor 4 Pressure"];

for i = 1:height(plotVars1)
    subplot(1,5,i)
    plot(t*1000,plotVars1(i,:),'LineWidth',lw,'DisplayName','Front')
    hold on
    plot(t*1000,plotVars2(i,:),'LineWidth',lw,'DisplayName','Rear')
    grid on
    xlabel("Time (ms)")
    title(titleLabs(i))
    if i == 1
...
impulseFig = figure(8);
impulseFig.Visible = 'off';
set(impulseFig,'Color','w','Units','inches','Position',[0 0 16 6])
set(groot, 'defaultAxesTickLabelInterpreter','latex'); set(groot, 'defaultLegendInterpreter','latex'); set(groot, 'defaultTextInterpreter','latex');

% Make a matrix of all plotted variables`
plotVars1 = [impulseBody(1,:); impulseMotor1(1,:); impulseMotor2(1,:); impulseMotor3(1,:); impulseMotor4(1,:)];
plotVars2 = [impulseBody(end,:); impulseMotor1(end,:); impulseMotor2(end,:); impulseMotor3(end,:); impulseMotor4(end,:)];
